...
for gene in gene_df[gene_df.tissue_module == '6'].index[:3]:
...
adata.obs['tm_6'] = adata.obsm['tm_pseudo_expression']['tm_6']
sc.pl.spatial(adata, color='tm_6')
adata.obs['tm_6_binary'] = adata.obsm['tm_binary']['tm_6']
sc.pl.spatial(adata, color='tm_6_binary')
adata.uns['tm_1_binary_colors'] = ['#CCCCCC', '#CA1C1C']
sc.pl.spatial(adata)
sc.pl.spatial(adata, color='tm_6_binary')
# Biological pathway enrichment analysis for detected tissue modules
tm_gene_list = tm_df[tm_df.tissue_module == '6'].index.tolist()
