...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 16 21:37:10 2021
...
def get_laplacian_mtx(adata, 
    ...
    """
    if not spatial_key == None:
        if not spatial_key in adata.obsm_keys():
            raise KeyError("%s is not avaliable in adata.obsm_keys" % \
                           spatial_key)
        adj_mtx = kneighbors_graph(adata.obsm[spatial_key], 
                                    n_neighbors=num_neighbors)
    else:
        if not set(spatial_names) <= set(adata.obs_keys()):
            raise KeyError("%s is not avaliable in adata.obs_keys" % \
                           spatial_names)
        adj_mtx = kneighbors_graph(adata.obs[spatial_names], 
                                    n_neighbors=num_neighbors)
...
def select_svg_normal(gene_score, num_sigma=1):
    mu = np.mean(gene_score['smooth_score'])
    sigma = np.std(gene_score['smooth_score'])
    gene_score['spatially_variable'] = 0
    gene_score.loc[gene_score['smooth_score'] > mu + num_sigma * sigma, 
