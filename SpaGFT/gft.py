...
def calculate_frequency_domain(adata,
    ...
    ratio_neighbors: float | 'infer', optional
        The ratio_neighbors will be used to determine the number of neighbors
        when construct the KNN graph by spatial coordinates. Indeed, ratio_neighbors * sqrt(number of spots)
        / 2 indicates the K. If 'infer', the para
        will be set to 1.0. The default is 'infer'.
...
def freq2umap(adata,
    ...
    calculate_frequency_domain(ftup_adata, ratio_low_freq=ratio_low_freq, ratio_high_freq=ratio_high_freq,
...
def identify_ftu(adata,
    ...
    **kwargs : kwargs 
    ...
    adata.uns['detect_ftu_data']['freq_domain_svgs'] = \
        ftup_adata[:, svg_list].varm['freq_domain']
    ...
    if isinstance(resolution, list):
        ...
        for resolution_index, resolution_value in enumerate(resolution):
            ...
            gft_adata_current.obs.louvain = [str(eval(i_ftu) + 1) for i_ftu in \
            ...
            for ftu in all_ftus_current:
                ...
                predict_ftu = KMeans(n_clusters=2,
            ...
            if count_ftus_select_cs_score > overlap_cs_score or \
                    resolution_index == 0:
    ...
    adata.uns['detect_ftu_data']['gft_umap_ftu'] = \
        pd.DataFrame(gft_adata.obsm['X_umap'],
                     index=gft_adata.obs.index,
                     columns=['UMAP_1', 'UMAP_2'])
    gft_adata.uns['gft_genes_ftu'] = [str(eval(i_ftu) + 1) for i_ftu in \
                                      gft_adata.obs.louvain.tolist()]
    gft_adata.obs.louvain = [str(eval(i_ftu) + 1) for i_ftu in \
                             gft_adata.obs.louvain.tolist()]
    ...
    for ftu in all_ftus:
        pseudo_exp = ftup_adata[:,
                     gft_adata.obs.louvain[gft_adata.obs.louvain == ftu].index] \
            .X.sum(axis=1)
        pseudo_exp = np.ravel(pseudo_exp)
        pseudo_df['ftu_' + str(ftu)] = pseudo_exp.copy()
        predict_ftu = KMeans(n_clusters=2, random_state=random_state) \
        ...
        cluster_middle_param = sum(abs(predict_ftu - pseudo_exp_cluster))
        cluster_middle_param_reverse = sum(abs(predict_ftu - \
    ...
    adata.uns['detect_ftu_data']['low_freq_domain_svg'] = \
    ...
    if not single_resolution:
        adata.uns['detect_ftu_data']['overlap_curve'] = \
            pd.DataFrame(overlap_scores,
                         index=['score'])
        adata.uns['detect_ftu_data']['overlap_curve'] = \
            adata.uns['detect_ftu_data']['overlap_curve'].transpose()
